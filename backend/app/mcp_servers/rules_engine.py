...
async def rules_engine(transaction_id: str) -> dict:
    ...
    if txn["amount"] < 5.0 and txn["type"] == "PAYMENT":
        fired.append({**RULES[0], "matched": True, "detail": f"Amount ${txn['amount']:.2f} < $5 threshold"})

    # Rule 2: Round-amount CASH_OUT
    if txn["type"] == "CASH_OUT" and txn["amount"] % 1000 == 0 and txn["amount"] >= 1000:
        fired.append({**RULES[1], "matched": True, "detail": f"Round CASH_OUT of ${txn['amount']:,.0f}"})
    ...
    if txn.get("country_code") in suspicious_countries:
        fired.append({**RULES[2], "matched": True, "detail": f"Country {txn['country_code']} + device {txn.get('device_fingerprint', 'unknown')[:8]}..."})

    # Rule 4: Balance drain
    if txn["old_balance_org"] and txn["old_balance_org"] > 0:
        drain_pct = (txn["old_balance_org"] - txn["new_balance_orig"]) / txn["old_balance_org"]
        if drain_pct >= 0.95 and txn["amount"] > 1000:
            fired.append({**RULES[3], "matched": True, "detail": f"Balance drained {drain_pct*100:.0f}%: ${txn['old_balance_org']:,.2f} → ${txn['new_balance_orig']:,.2f}"})

    # Rule 5: Merchant category drift
    drift_categories = {"crypto", "gambling", "gaming"}
    if txn.get("merchant_category") in drift_categories:
        fired.append({**RULES[4], "matched": True, "detail": f"Unusual category: {txn['merchant_category']}"})
