      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            {total.toLocaleString()} transactions {fraudOnly ? '(flagged only)' : ''}
          </p>
        </div>
        <div className="flex items-center gap-4">
          <label className="flex items-center gap-2 cursor-pointer">
            <div
              ...
              onClick={() => { setFraudOnly(!fraudOnly); setPage(0) }}
            >
              <div className={`w-5 h-5 bg-white rounded-full shadow transition-transform ${fraudOnly ? 'translate-x-5' : ''}`} />
            </div>
            ...
            <tbody>
              {loading ? (
                ...
                transactions.map((txn) => (
                  <tr key={txn.transaction_id} className="border-b border-[#1a1a1a] hover:bg-[#1a1a1a] transition-colors">
                    <td className="px-4 py-3 font-mono text-xs">{txn.transaction_id}</td>
                    <td className="px-4 py-3">
                      <span className="px-2 py-0.5 rounded bg-[#1e1e1e] text-xs">{txn.type}</span>
                    </td>
                    <td className="px-4 py-3 text-right font-mono">${txn.amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                    <td className="px-4 py-3 text-xs text-gray-400">{txn.name_orig}</td>
                    <td className="px-4 py-3 text-xs text-gray-400">{txn.name_dest}</td>
                    <td className="px-4 py-3 text-xs">{txn.merchant_category}</td>
                    ...
                    <td className="px-4 py-3">
                      ...
                      >
                        {investigating === txn.transaction_id ? 'Starting...' : 'Investigate'}
                      </button>
                    </td>
                  </tr>
                ))
