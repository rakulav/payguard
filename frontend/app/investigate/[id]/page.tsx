...
export default function InvestigatePage() {
  ...
  useEffect(() => {
    ...
    es.addEventListener('tool_result', (e) => {
      const data = JSON.parse(e.data)
      setEvents(prev => [...prev, data])
      if (data.content && typeof data.content === 'string' && data.content.includes('similar')) {
        try {
          const parsed = JSON.parse(data.content)
          if (parsed.similar_transactions) setSimilarFrauds(parsed.similar_transactions)
        } catch {}
      }
    })
      ...
      {invMeta?.cost_usd != null && (
        <div className="text-sm text-gray-400 px-1">
          ...
          {invMeta.model_breakdown?.length ? (
            <span className="ml-3">
              ({invMeta.model_breakdown.map((m) => `${m.agent}:${m.model}`).join(' · ')})
            </span>
          ) : null}
        </div>
      )}
        ...
        {transaction ? (
          <div className="space-y-3 text-sm">
            ...
            <div>
              ...
              <p><span className="px-2 py-0.5 rounded bg-[#1e1e1e] text-xs">{transaction.type}</span></p>
            </div>
            <div>
              <span className="text-gray-500">Amount</span>
              <p className="text-lg font-bold">${transaction.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-mono text-xs">
                ${transaction.old_balance_org.toLocaleString()} → ${transaction.new_balance_orig.toLocaleString()}
              </p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-mono text-xs truncate">{transaction.device_fingerprint}</p>
            </div>
            <div>
              <span className="text-gray-500">Status</span>
              <p>{transaction.is_fraud ? <span className="badge-fraud">Fraud</span> : <span className="badge-legit">Clean</span>}</p>
            </div>
          </div>
        ) : (
        ...
        {panelTab === 'audit' ? (
          <>
            ...
            {auditEntries.length === 0 ? (
              <p className="text-gray-500 text-sm">No audit rows yet. Wait for the investigation to finish.</p>
            ) : (
              <ul className="space-y-3 text-sm">
                {auditEntries.map((e) => (
                  <li key={e.id} className="border-l-2 border-gray-700 pl-3">
                    ...
                    {e.reason ? <p className="text-gray-500 text-xs mt-1">{e.reason}</p> : null}
                  </li>
                ))}
              </ul>
            )}
          ...
          <>
            <div className="flex items-center justify-between mb-4">
              ...
              {done ? (
                <span className="badge bg-green-900/50 text-green-300 border border-green-800">Complete</span>
              ) : (
                <span className="badge bg-blue-900/50 text-blue-300 border border-blue-800 animate-pulse">Running...</span>
              )}
            </div>
