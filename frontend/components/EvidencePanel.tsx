                    ...
                    {content.anomaly_score !== undefined && (
                      <div className="flex justify-between">
                        ...
                        <span className="text-amber-400">
                          {(content.anomaly_score * 100).toFixed(0)}%
                        </span>
                      </div>
                    )}
                    {content.behavioral_flags && (
                      <div className="mt-2">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {content.behavioral_flags.map(
                            (flag: string, j: number) => (
                              <span
                                key={j}
                                className="px-1.5 py-0.5 bg-amber-900/30 text-amber-300 rounded text-[10px]"
                              >
                                {flag}
                              </span>
                            ),
      ...
      {toolResults.length > 0 && (
        <div className="mb-4">
          ...
          {toolResults.map((event, i) => (
            <div
              key={i}
              className="bg-[#0d0d0d] rounded p-2 mb-1 text-xs text-gray-400"
            >
          ...
          {similarFrauds.slice(0, 5).map((fraud, i) => (
            <div key={i} className="bg-[#0d0d0d] rounded p-2 mb-1 text-xs">
              <div className="flex justify-between">
                ...
                <span
                  className={fraud.is_fraud ? "text-red-400" : "text-green-400"}
                >
                  {fraud.is_fraud ? "Fraud" : "Clean"}
                </span>
              </div>
              <div className="text-gray-500 mt-0.5">
                {fraud.type} · ${fraud.amount?.toLocaleString()} ·{" "}
                {fraud.country_code}
              </div>
              {fraud.sources && (
